...
所有原生 C++ 目标均使用静态 MSVC 运行库（Release 为 `/MT`，Debug 为 `/MTd`）。
`ShuruIme.dll` 与 `ShuruIme32.dll` 均不依赖目标机器的 `MSVCP140.dll`、
`VCRUNTIME140.dll` 或 `VCRUNTIME140_1.dll`；设置程序把 .NET 8 Desktop Runtime
一同发布，因此目标电脑无需另装 VC++ Redistributable 或 .NET Desktop Runtime。
