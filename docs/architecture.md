...
- `clipboard/history.db`：剪贴板 SQLite 主库，使用 WAL、时间索引和 FTS5 trigram 搜索
- `clipboard/images/`：剪贴板图片原始 PNG；数据库只保存元数据与受管路径
- 系统词库从版本化的 `%ProgramData%\CaishenPinyin\data\lexicon` 加载

### 2.3.1 系统词库冷加载（EngineSnapshot v2）

系统词库是进程内状态，每个宿主进程（SearchHost、记事本等）都要独立完成一次"就绪"。
为让首次按键与热态体感一致，冷加载分三层：

1. **快照优先**：`PinyinEngine::Initialize` 首先尝试 `%LOCALAPPDATA%\CaishenPinyin\snapshot\`
   下的 `EngineSnapshot v2` 只读快照（`src/engine/engine_snapshot.cpp`）。快照把传统装载的
   最终产物——排序后的词条、键哈希索引、拼音 Trie、音节 Trie、音节表、词指纹、英文词典——
   序列化为偏移量布局，加载即 `CreateFileMapping` + 头部/结构校验。语言模型不进快照（本就是
   mmap）。在一台 Windows 11 开发机的 Release 构建中，白霜全量词库（67.7 万词条）传统
   装载约 11-12 秒，快照装载后引擎整体约 150-160 毫秒就绪（其中映射和校验约 80 毫秒）。
   该数据用于说明优化方向，不代表所有硬件和宿主环境的性能保证。
   校验采用顺序边界扫描保持预取友好；Trie 环防护由查询侧兄弟链跳数上限兜底，
   映射后立即 `PrefetchVirtualMemory` 重叠磁盘等待。
2. **失败回退**：快照缺失、源文件 size/mtime 不匹配或结构校验失败时，走原有文本/.bin 缓存
   流式装载路径；装载成功并在发布之后，同一后台线程把新快照写入缓存（临时文件 + 原子替换，
   文件名含源标识 tag，词库升级后旧 tag 自然失效并顺带清理）。生成失败（如 AppContainer
   宿主无写权限）静默放弃。安装包场景下，安装器会在部署成功后以隐藏方式调用随包的
   `engine_snapshot_build_tool.exe` 预先完成这一步，用户首次打字即命中快照
   （详见 `docs/installer.md`）。
3. **有界等待代替原始拼音保底**：引擎未就绪时按键路径调用 `SharedEngine::WaitForReady`
   （上限 300ms）；等待期间组合串照常显示但不产生也不提交任何候选。仅当加载真正失败才
   退回原始拼音异常保底。快照装载后引擎就绪时间低于该等待上限，正常首键无感知。

完整性策略：头部记录各源文件的 size+mtime 与生成时 SHA-256；加载路径只 stat 比对
size+mtime 并做一遍 O(n) 结构校验遍（索引有序性、Trie 无环、区段边界），不在每次加载
重算 40MB 级强哈希。一致性由 `engine_snapshot` 测试保证：同一词库走堆装载与映射装载，
五类查询结果必须逐字段一致。

### 2.4 设置层（`settings`）

WPF 程序分为常规设置、输入方案、自定义短语、剪贴板、词库与隐私五页。用户界面只显示“全拼 / 双拼”。设置与短语均使用临时文件写通后原子替换；自定义短语还使用跨进程互斥，避免输入法与设置程序并发读写产生半文件。

剪贴板监听由单实例隐藏设置进程承担，避免在每个加载 TSF DLL 的宿主中重复监听。文本、文件和图片元数据写入 SQLite；旧版 JSON 只在首次启动时迁移。图片优先读取剪贴板原生 PNG，检测到所有 Alpha 为 0 且仍有 RGB 内容时才恢复不透明，正常半透明图片保持原样。`v` 候选中的图片通过稳定记录 ID 交给辅助进程，以 `PNG + Bitmap` 格式写回剪贴板并粘贴到原窗口，不把图片路径提交成文本。

独立搜索窗口由目标进程中的 `TextService` 创建一次性直接上屏会话：128 位随机
令牌只定位当前会话，文本正文由设置程序以固定头、严格 UTF-8、16 MiB 上限和
临时文件原子改名写入 `%LOCALAPPDATA%\CaishenPinyin\direct_commit_requests\`。
目标进程在自己的 TSF UI 线程中确认原窗口已重新成为前台、上下文仍属于原目标且
不是敏感 InputScope 后，调用既有 `CommitText` 编辑会话上屏，并写回一次性结果。
因此文本、长文本、文件路径和自定义短语不再依赖系统剪贴板；窗口关闭会发布取消
标记，超时或上下文变化不会在稍后的意外时刻补写。请求目录采用当前用户受保护
DACL，并显式拒绝 AppContainer。AppContainer 宿主保持旧窗口回退，不传递令牌。

图片仍必须通过 Windows 系统剪贴板向任意应用传递。第三方程序若长期持有
`OpenClipboard`，图片粘贴会在后台 STA 有界重试后明确失败，不发送使用旧剪贴板
内容的 `Ctrl+V`，也不会终止或注入占用剪贴板的进程。
